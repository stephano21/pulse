...
[ApiController]
[ApiVersion("1.0")]
[Route("v{version:apiVersion}/unidades")]
[Authorize]
public sealed class UnidadesController(ISyncService sync) : ControllerBase
{
    [HttpGet]
    ...
    {
        var tid = TenantId();
        var result = await sync.PullUnidadesAsync(tid, updatedSince, cursor, limit, ct);
        return Ok(result);
    }

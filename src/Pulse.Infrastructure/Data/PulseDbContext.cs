    ...
    : IdentityDbContext<ApplicationUser, IdentityRole<Guid>, Guid>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            e.ToTable("refresh_tokens", DatabaseSchemas.Pulse);
            e.HasKey(x => x.Id);
        ...
        {
            e.ToTable("unidades_medida", DatabaseSchemas.Pulse);
            e.HasKey(x => x.Id);
            e.Property(x => x.Nombre).HasMaxLength(512);
            e.HasIndex(x => new { x.TenantId, x.UpdatedAt });
            e.HasOne(x => x.Tenant).WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict);
        ...
        modelBuilder.Entity<UnidadMedidaLocalMapping>(e =>
        {
            e.ToTable("unidad_medida_local_mappings", DatabaseSchemas.Pulse);
            e.HasKey(x => x.Id);
            e.HasIndex(x => new { x.TenantId, x.LocalId }).IsUnique();
            e.HasOne(x => x.Unidad).WithMany().HasForeignKey(x => x.UnidadId).OnDelete(DeleteBehavior.Cascade);
        });
    }
}
